...
#' )
#'
glm_log_mult <- function(data, outcomes, predictors, covariates="", formulas = NULL, exponentiate = TRUE, progress = FALSE, std_odds = FALSE) {
  #create data frame with regression formulas
  construct_formulas(outcomes = outcomes, predictors = predictors, covariates = covariates, formulas = formulas, randoms = "") -> df
  ...
  df %>%
    ...
    dplyr::mutate(
      ...
      model_result = purrr::map(model_result, .f = purrr::possibly(~broom::tidy(.x, exponentiate = exponentiate, conf.int = FALSE)), .progress = ifelse(progress, "Making output pretty", FALSE))
      ...
      df %>%
        dplyr::mutate(
          ...
          std_model = purrr::map2(.x = formula, .y = std_data,
                           ...
                           .progress = ifelse(progress, "Adding standardized predictors", FALSE)),
          std_confint = purrr::map(std_model, .f = purrr::possibly(~confint.default(.x))),
          std_model = purrr::map(std_model, purrr::possibly(~broom::tidy(.x, exponentiate = FALSE, conf.int = FALSE)))
          # unnest each model so all models form a single table
        ) %>% tidyr::unnest(cols = c(std_model, std_confint)) %>%
        dplyr::mutate(std_confint = dplyr::as_tibble(std_confint, .names_repair = "check_unique") %>%
                 dplyr::rename_with( ~dplyr::case_when(
                   stringr::str_detect(.x, "2.5 %|V1") ~ "conf.low",
                   stringr::str_detect(.x, "97.5 %|V2") ~ "conf.high"
                 ))
