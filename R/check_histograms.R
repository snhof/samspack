...
#'
check_histograms <- function (data, facet_rows = NULL, facet_cols = NULL, id = dplyr::row_number(), bins_method = "sturges") {
purrr::map(
  .x = data %>% dplyr::select(dplyr::where(is.numeric), -any_of(as.character(substitute({{id}})))) %>% names(),
